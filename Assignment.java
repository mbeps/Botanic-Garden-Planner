...
class GardenItemStore {
    ...
    public void put(String key, String plant) {
        this.map.put(key, new ArrayList<String>()); // Create map with one key and empty array 
        this.map.get(key).add(plant); // Get the map first and then add element to array
    ...
    public String getRandomItem(String key) { //* Returns random plant by randomly selecting index
        ...
        return (array.get(randomIndex)); //! BUG: Return last item in the array 
    }
